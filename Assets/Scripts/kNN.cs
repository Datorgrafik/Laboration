...
public class KNN : MonoBehaviour
{
	...
	public object Vote(IndexAndDistance[] info, List<Dictionary<string, object>> trainData, int k)
	{
		...
		for (int i = 0; i < k; ++i)
		{
			...
			int idx = info[i].idx;
			// Class in last cell
			string c = (string)trainData[idx][attributes[attributes.Count - 2]];
	...
	public object VoteReg(IndexAndDistance[] info, List<Dictionary<string, object>> trainData, int k)
	{
		kPoints = new List<int>();
		double sum = 0.0;
	...
	public object WeightClass(IndexAndDistance[] info, List<Dictionary<string, object>> trainData, int k)
	{
		// One cell per class
		...
		for (int i = 0; i < k; ++i)
		{
			...
			int idx = info[i].idx;
			// Class in last cell
			string c = (string)trainData[idx][attributes[attributes.Count - 1]];
	...
	public object WeightReg(IndexAndDistance[] info, List<Dictionary<string, object>> trainData, int k)
	{
